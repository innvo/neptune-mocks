...
def convert_organization_address_edges():
    try:
        # Ensure output directory exists
        os.makedirs('src/data/output/neptune', exist_ok=True)
        
        # Read the mock organization-address data from JSON
        print("Reading mock organization-address data...")
        ...
        print("\nConverting organization-address edges to Gremlin format...")
        ...
        output_path = 'src/data/output/neptune/neptune_organization-address_edges_gremlin.csv'
        edges_df.to_csv(output_path, index=False, quoting=1, quotechar='"', escapechar='\\')
        
        print(f"\nGenerated {len(edges)} organization-address edges")
        print(f"Saved to {output_path}")
        return True
        
    except Exception as e:
        print(f"Error converting organization-address edges: {str(e)}")
        return False

def convert_to_gremlin():
    # Convert organization-address edges
    edges_success = convert_organization_address_edges()
