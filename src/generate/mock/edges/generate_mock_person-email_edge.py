...
def process_person_batch(batch_data):
    """Process a batch of persons to generate email edges"""
    person_ids, email_ids, edge_counts, batch_start_idx = batch_data
    batch_edges = []
    used_pairs = set()
    
    for i, person_id in enumerate(person_ids):
        # Get available emails that haven't been used with this person
        available_emails = [email_id for email_id in email_ids 
                          if (person_id, email_id) not in used_pairs]
        
        # If no available emails, reuse some (ensures every person gets at least 1 edge)
        if len(available_emails) == 0:
            available_emails = email_ids.copy()
        
        # Use pre-generated edge count for this person
        num_email_edges = edge_counts[batch_start_idx + i]
...
def generate_person_email_edges():
    ...
    try:
        ...
        print("\nGenerating person_email edges with optimized processing...")
        ...
        for i in range(0, total_persons, batch_size):
            ...
            batches.append((batch_person_ids, email_ids, edge_counts, i))
        ...
        for count in sorted(edges_per_person_dist.keys()):
            print(f"Persons with {count} email edges: {edges_per_person_dist[count]}")
        
        # Print email type statistics
        ...
        print(f"Persons with too many email edges (>5): {validation_results['persons_with_too_many_edges']}")
        ...
        print(f"Error generating person-email edges: {str(e)}")
        import traceback
        traceback.print_exc()
        return None
