...
def process_person_chunk_streaming(chunk_data):
    ...
    person_chunk, email_ids, edge_counts_chunk, chunk_start_idx = chunk_data
    ...
    edge_idx = 0
    email_type_choices = ['SECONDARY', 'WORK', 'PERSONAL']
    email_ids_len = len(email_ids)
    ...
    for i, person_id in enumerate(person_chunk):
        ...
        num_edges = max(1, min(num_edges, email_ids_len, max_edges_per_person))
        
        # Ultra-fast random sampling using numpy
        if num_edges == 1:
            selected_idx = np.random.randint(0, email_ids_len)
            selected_emails = [email_ids[selected_idx]]
        elif num_edges <= email_ids_len:
            selected_indices = np.random.choice(email_ids_len, size=num_edges, replace=False)
            selected_emails = [email_ids[idx] for idx in selected_indices]
        else:
            selected_emails = email_ids[:num_edges]
        
        # Generate edges with minimal object creation
        for j, email_id in enumerate(selected_emails):
            email_type = 'PRIMARY' if j == 0 else random.choice(email_type_choices)
...
def generate_person_email_edges_streaming():
    """
    ...
    try:
        ...
        print(f"🚀 STREAMING PERSON-EMAIL EDGE GENERATION")
        ...
        email_mask = node_df['node_type'] == 'email'
        
        person_ids = node_df[person_mask]['node_id'].tolist()
        email_ids = node_df[email_mask]['node_id'].tolist()
        
        print(f"\nDataset size: {len(person_ids):,} persons → {len(email_ids):,} emails")
        
        if not person_ids or not email_ids:
            raise ValueError("Missing required node types")
        
        # Pre-generate edge distribution
        print("Generating edge distribution...")
        edge_counts = np.random.choice([1, 2, 3], size=len(person_ids), p=[0.6, 0.3, 0.1])
        ...
        for chunk_idx in tqdm(range(total_chunks), desc="Processing chunks"):
            ...
            # Process this chunk
            chunk_data = (person_chunk, email_ids, edge_counts_chunk, start_idx)
        ...
        email_type_stats = {'PRIMARY': 0, 'SECONDARY': 0, 'WORK': 0, 'PERSONAL': 0}
        ...
        output_path = 'src/data/output/gds/mock_person-email_data.json'
        ...
        for edge in sample_edges:
            if edge['node_id_from'] in person_node_ids and edge['node_id_to'] in email_node_ids:
                valid_sample += 1
        ...
        # Results summary
        print("✅ STREAMING PERSON-EMAIL EDGE GENERATION COMPLETE")
        print("=" * 60)
...
def generate_person_email_edges():
    ...
    return generate_person_email_edges_streaming()

if __name__ == "__main__":
    generate_person_email_edges()