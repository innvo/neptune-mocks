...
def process_person_chunk_streaming(chunk_data):
    ...
    person_chunk, address_ids, edge_counts_chunk, chunk_start_idx = chunk_data
    ...
    edge_idx = 0
    address_type_choices = ['SECONDARY', 'TERTIARY']
    ...
    for i, person_id in enumerate(person_chunk):
        ...
        num_edges = max(1, min(num_edges, address_ids_len, max_edges_per_person))
        
        # Ultra-fast random sampling using numpy
        if num_edges == 1:
            selected_idx = np.random.randint(0, address_ids_len)
            selected_addresses = [address_ids[selected_idx]]
        elif num_edges <= address_ids_len:
            selected_indices = np.random.choice(address_ids_len, size=num_edges, replace=False)
            selected_addresses = [address_ids[idx] for idx in selected_indices]
        else:
            selected_addresses = address_ids[:num_edges]
        
        # Generate edges with minimal object creation
        for j, address_id in enumerate(selected_addresses):
            address_type = 'PRIMARY' if j == 0 else random.choice(address_type_choices)
...
def generate_person_address_edges_streaming():
    """
    ...
    try:
        ...
        print(f"🚀 STREAMING PERSON-ADDRESS EDGE GENERATION")
        ...
        node_df = pd.read_csv('src/data/input/node_data.csv', 
                            ...
                            engine='c',
                            memory_map=True)  # Read entire file into memory
        ...
        print(f"\nDataset size: {len(person_ids):,} persons → {len(address_ids):,} addresses")
        
        if not person_ids or not address_ids:
        ...
        for chunk_idx in tqdm(range(total_chunks), desc="Processing chunks"):
            ...
            # Process this chunk
            chunk_data = (person_chunk, address_ids, edge_counts_chunk, start_idx)
        ...
        output_path = 'src/data/output/gds/mock_person-address_data.json'
        ...
        for edge in sample_edges:
            if edge['node_id_from'] in person_node_ids and edge['node_id_to'] in address_node_ids:
                valid_sample += 1
        ...
        # Results summary
        print("✅ STREAMING PERSON-ADDRESS EDGE GENERATION COMPLETE")
        print("=" * 60)
        ...
        print(f"\n🏷️  ADDRESS TYPE DISTRIBUTION:")
        for addr_type, count in address_type_stats.items():
            print(f"{addr_type}: {count:,} edges")
...
def generate_person_address_edges():
    ...
    return generate_person_address_edges_streaming()
