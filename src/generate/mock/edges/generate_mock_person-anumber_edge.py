...
def generate_person_anumber_edges():
    try:
        ...
        print("\nGenerating person_anumber edges...")
        for _, person in tqdm(person_nodes.iterrows(), total=len(person_nodes), desc="Processing person nodes"):
            ...
            num_anumber_edges = min(num_anumber_edges, len(anumber_nodes))
        ...
        if validation_results['missing_to_nodes']:
            print(f"\nMissing or invalid anumber nodes: {len(validation_results['missing_to_nodes'])}")
            print("Sample of missing anumber nodes:", list(validation_results['missing_to_nodes'])[:5])
        ...
        print(f"Total number of person_anumber edges generated: {edge_type_count}")
        ...
        with open('src/data/output/gds/mock_person-anumber_data.json', 'r') as f:
...
if __name__ == "__main__":
    edge_df = generate_person_anumber_edges()
