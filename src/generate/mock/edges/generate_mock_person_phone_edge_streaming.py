...
def process_person_chunk_streaming(chunk_data):
    ...
    person_chunk, phone_ids, edge_counts_chunk, chunk_start_idx = chunk_data
    ...
    edge_idx = 0
    phone_type_choices = ['SECONDARY', 'MOBILE', 'HOME', 'WORK']
    phone_ids_len = len(phone_ids)
    ...
    for i, person_id in enumerate(person_chunk):
        ...
        num_edges = max(1, min(num_edges, phone_ids_len, max_edges_per_person))
        
        # Ultra-fast random sampling using numpy
        if num_edges == 1:
            selected_idx = np.random.randint(0, phone_ids_len)
            selected_phones = [phone_ids[selected_idx]]
        elif num_edges <= phone_ids_len:
            selected_indices = np.random.choice(phone_ids_len, size=num_edges, replace=False)
            selected_phones = [phone_ids[idx] for idx in selected_indices]
        else:
            selected_phones = phone_ids[:num_edges]
        
        # Generate edges with minimal object creation
        for j, phone_id in enumerate(selected_phones):
            phone_type = 'PRIMARY' if j == 0 else random.choice(phone_type_choices)
...
def generate_person_phone_edges_streaming():
    """
    ...
    try:
        ...
        print(f"🚀 STREAMING PERSON-PHONE EDGE GENERATION")
        ...
        phone_mask = node_df['node_type'] == 'phone'
        
        person_ids = node_df[person_mask]['node_id'].tolist()
        phone_ids = node_df[phone_mask]['node_id'].tolist()
        
        print(f"\nDataset size: {len(person_ids):,} persons → {len(phone_ids):,} phones")
        
        if not person_ids or not phone_ids:
        ...
        for chunk_idx in tqdm(range(total_chunks), desc="Processing chunks"):
            ...
            # Process this chunk
            chunk_data = (person_chunk, phone_ids, edge_counts_chunk, start_idx)
        ...
        phone_type_stats = {'PRIMARY': 0, 'SECONDARY': 0, 'MOBILE': 0, 'HOME': 0, 'WORK': 0}
        ...
        output_path = 'src/data/output/gds/mock_person-phone_data.json'
        ...
        for edge in sample_edges:
            if edge['node_id_from'] in person_node_ids and edge['node_id_to'] in phone_node_ids:
                valid_sample += 1
        ...
        # Results summary
        print("✅ STREAMING PERSON-PHONE EDGE GENERATION COMPLETE")
        print("=" * 60)
...
def generate_person_phone_edges():
    ...
    return generate_person_phone_edges_streaming()
