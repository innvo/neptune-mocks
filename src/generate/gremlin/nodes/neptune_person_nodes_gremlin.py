...
def generate_name_variants(full_name):
    """Generate name variations based on full name"""
    if not full_name:
        return []
...
def generate_mock_person_batch(batch_size):
    ...
    for _ in range(batch_size):
        ...
        # Format arrays properly for Neptune using semicolon delimiters
        ...
        node = {
            ...
            #'name_full_list:String[]': name_full_list_formatted,
            'date_of_birth:Date': birth_date_str,
            #'date_of_birth_list:Date[]': date_of_birth_list_formatted,
            'anumber_primary:String': anumber_primary,
            #'anumber_list:String[]': anumber_list_formatted,
            '~label': 'person'
        }
...
def generate_mock_person_data(num_records=1000):
...
def save_to_csv(nodes, output_path):
    ...
    column_order = ['~id',
                    ...
                    # 'node_name:String', 
                    # 'name_full:String', 
                    # 'name_full_list:String[]', 
                    # 'date_of_birth:Date', 
                    # 'date_of_birth_list:Date[]',
                    # 'anumber_primary:String', 
                    # 'anumber_list:String[]',
                     '~label']
