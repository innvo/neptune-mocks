...
def get_subgraph(node_ids: List[str]) -> List[Dict[str, Any]]:
    """Get subgraph for multiple nodes (person or address) and their connections up to 2 levels deep."""
    # Format the list of node IDs for the query
    node_ids_str = ', '.join([f'"{id}"' for id in node_ids])
    query = f'MATCH path = (start)-[*0..3]-(n) WHERE id(start) IN [{node_ids_str}] RETURN nodes(path) as nodes, relationships(path) as edges'
    result = query_neptune(query)
    return result.get('results', [])

def visualize_network(node_ids: List[str]):
    """
    ...
    try:
        ...
        logger.info(f"Fetching subgraph for nodes {node_ids} from Neptune...")
        subgraph_data = get_subgraph(node_ids)
...
if __name__ == "__main__":
    ...
    print(f"\nVisualizing network for node IDs: {node_ids}")
    visualize_network(node_ids) 