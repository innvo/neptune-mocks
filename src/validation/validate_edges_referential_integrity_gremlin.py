...
def validate_person_phone_edges():
    try:
        # Read the node data
        print("\nReading person nodes...")
        person_nodes_df = pd.read_csv('src/data/output/neptune/neptune_person_nodes_gremlin.csv')
        ...
        print("\nReading phone nodes...")
        phone_nodes_df = pd.read_csv('src/data/output/neptune/neptune_phone_nodes_gremlin.csv')
        phone_ids = set(phone_nodes_df['~id'])
        print(f"Found {len(phone_ids)} unique phone nodes")
        
        # Read the edge data
        print("\nReading person-phone edges...")
        edges_df = pd.read_csv('src/data/output/neptune/neptune_person_phone_edges_gremlin.csv')
        print(f"Found {len(edges_df)} edges to validate")
        ...
        # Check for missing target nodes (phone IDs)
        missing_to = set(edges_df['~to']) - phone_ids
        if missing_to:
            print(f"\nERROR: Found {len(missing_to)} edges with missing target nodes:")
            for node_id in missing_to:
                print(f"  - Edge target node {node_id} not found in phone nodes file")
        
        # Print summary
        total_errors = len(missing_from) + len(missing_to)
        if total_errors == 0:
            print("\nSUCCESS: All person-phone edges have valid source and target nodes!")
        else:
            print(f"\nFAILURE: Found {total_errors} referential integrity errors in person-phone edges")
            print(f"  - {len(missing_from)} missing source nodes")
            print(f"  - {len(missing_to)} missing target nodes")
...
def validate_edges():
    ...
    validation_results['person_phone'] = validate_person_phone_edges()
